...
function getStatusVariant(status: string): 'active' | 'inactive' | 'suspended' | 'info' {
  switch (status) { case 'Activo': return 'active'; case 'Inactivo': return 'inactive'; case 'Suspendido': return 'suspended'; default: return 'info'; }
}
function Field({ label, value }: { label: string; value: React.ReactNode }) {
  return (<div><dt className="text-xs text-text-secondary font-medium uppercase tracking-wider mb-0.5">{label}</dt><dd className="text-sm text-text-primary">{value || '—'}</dd></div>);
}
...
export default function BeneficiaryDetailPage() {
  ...
  useEffect(() => {
    ...
    const bid = Number(id);
    Promise.all([
      beneficiaryService.getById(bid).then(res => { if (res.success && res.data) setBeneficiary(res.data); }),
      healthProfileService.getByBeneficiary(bid).then(res => { if (res.success && res.data) setHealthProfile(res.data); }).catch(() => {}),
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        ...
        {canManageBeneficiaries && (
          <div className="flex gap-2">
            <Link to={`/beneficiarios/${id}/editar`}><Button variant="secondary" size="sm"><FontAwesomeIcon icon={faPen} className="mr-1.5" />Editar</Button></Link>
            <Link to={`/beneficiarios/${id}/salud`}><Button variant="secondary" size="sm"><FontAwesomeIcon icon={faHeartPulse} className="mr-1.5" />Perfil de salud</Button></Link>
          </div>
        )}
      </div>
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card title="Información personal">
          <dl className="grid grid-cols-2 gap-4">
            ...
            <Field label="Código interno" value={<code className="bg-gray-100 px-1.5 py-0.5 rounded text-xs">{beneficiary.internalCode}</code>} />
            ...
            <Field label="Documento" value={beneficiary.identityDocument} />
            ...
            <Field label="Estado" value={<Badge variant={getStatusVariant(beneficiary.status)}>{beneficiary.status}</Badge>} />
            <Field label="Fecha registro" value={formatDate(beneficiary.registeredAt)} />
          </dl>
          {beneficiary.notes && (<div className="mt-4 pt-4 border-t border-gray-100"><Field label="Notas" value={beneficiary.notes} /></div>)}
        </Card>
        <Card title="Perfil de salud">
          {healthProfile ? (
            ...
            <div className="text-center py-8">
              ...
              {canManageBeneficiaries && <Link to={`/beneficiarios/${id}/salud`}><Button variant="secondary" size="sm">Crear perfil de salud</Button></Link>}
            </div>
          )}
        </Card>
      </div>
