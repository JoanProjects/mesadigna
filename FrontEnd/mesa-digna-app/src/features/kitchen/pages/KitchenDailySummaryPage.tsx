...
import {faArrowRight, faCircleInfo, faInbox, faUtensils} from '@fortawesome/free-solid-svg-icons';
import {usePageTitle} from '@/hooks/usePageTitle';
import {StatsCard} from '@/components/shared/StatsCard';
import {Badge, Button, EmptyState, Loader, Modal, Pagination, SearchInput} from '@/components/ui';
        ...
        <>
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
                ...
                <div className="flex gap-2 items-center">
                    ...
                    <Link to="/cocina/dietario"><Button variant="secondary" size="sm">Dietario <FontAwesomeIcon
                        icon={faArrowRight} className="ml-1"/></Button></Link>
                    <Link to="/cocina/operacional"><Button variant="secondary" size="sm">Operacional <FontAwesomeIcon
                        icon={faArrowRight} className="ml-1"/></Button></Link>
                </div>
            </div>
            ...
            {summary && (
                <>
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
                        <StatsCard icon={faUtensils} value={summary.totalServings} label="Total porciones"
                                   color="bg-primary-50 text-primary-500"/>
                        <StatsCard icon={faUtensils} value={summary.regularServings} label="Porciones regulares"
                                   color="bg-accent-50 text-accent-400"/>
                        <StatsCard icon={faUtensils} value={summary.specialDietServings} label="Porciones especiales"
                        ...
                        <div className="overflow-x-auto">
                            <table className="w-full">
                                ...
                                <tr className="border-b border-gray-100">
                                    ...
                                    <th className="text-right pr-4 py-3 text-xs font-semibold text-text-secondary uppercase tracking-wider">Acciones</th>
                                </tr>
                                </thead>
                                <tbody>
                                {summary.dietCategories.map(cat => (
                                    ...
                                    <tr key={cat.categoryKey}
                                        ...
                                        <td className=" py-3 text-sm text-right pr-4">
                                            <button onClick={() => openModal(cat.category, cat.categoryKey)}
                                                    className=" inline-flex items-center gap-1.5 text-sm text-primary-500 hover:text-primary-900 cursor-pointer bg-transparent border-none font-medium">
            ...
            <Modal open={modalOpen} onClose={closeModal} title={`Beneficiarios — ${modalCategory}`}>
                <div className="flex flex-col h-full w-full">
                    <div className="flex-shrink-0 mb-3">
                        <SearchInput value={modalSearch} onChange={setModalSearch}
                                     placeholder="Buscar por nombre o código..."/>

                        {modalTotalCount > 0 &&
                            <p className="text-xs text-text-light text-right pr-3 mt-1">{modalTotalCount} beneficiarios</p>}

                    </div>
                    <div className="flex-1 overflow-y-auto">
                        {modalLoading ? <Loader message="Cargando..."/> : filteredModal.length === 0 ? (
                            <EmptyState icon={faInbox} title="Sin beneficiarios"
                                        message="No hay beneficiarios en esta categoría."/>
                        ) : (
                            <table className="w-full">
                                <thead>
                                <tr className="border-b border-gray-100">
                                    <th className="text-left pl-20 py-2 text-xs font-semibold text-text-secondary uppercase">Código</th>
                                    <th className="pl-20 text-xs font-semibold text-text-secondary uppercase">Nombre</th>
                                </tr>
                                </thead>
                                <tbody>{filteredModal.map(b => (
                                    <tr key={b.beneficiaryId} className="border-b border-gray-50">
                                        <td className=" py-2 text-sm pl-18">
                                            <code className="bg-gray-100 py-0.5 px-1.5 rounded-lg text-xs">{b.internalCode}</code>
                                        </td>
                                        <td className=" pl-40 py-2 text-sm text-text-primary">{b.fullName}</td>
                                    </tr>
                                ))}</tbody>
                            </table>
                        )}
                    ...
                    {modalTotalPages > 1 && (
                        <div className="flex-shrink-0 pt-3 border-t border-gray-100">
                            <Pagination page={modalPage} totalPages={modalTotalPages}
                                        onPageChange={p => fetchModalPage(p)}/>
                        </div>
                    )}
                </div>
            </Modal>
