...
export default function MealFormPage() {
  ...
  const { values, errors, serverError, setServerError, setValues, setErrors, validate, handleChange } = useForm(mealSchema, {
  ...
  useEffect(() => {
    ...
    mealService.getById(Number(id)).then(res => {
      if (res.success && res.data) {
        const m = res.data;
        setValues({ name: m.name, description: m.description || '', mealType: m.mealType, baseServings: m.baseServings });
      }
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isEdit) {
        ...
        if (res.success) { notify('Comida actualizada correctamente.'); navigate(`/comidas/${id}`); }
        else setServerError(res.message || 'Error al actualizar.');
      } else {
        const res = await mealService.create(values);
        if (res.success && res.data) { notify('Comida creada correctamente.'); navigate(`/comidas/${res.data.id}`); }
      ...
      <Card>
        <form onSubmit={onSubmit} className="space-y-4 max-w-2xl">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <Select label="Tipo de comida" value={values.mealType} onChange={handleChange('mealType')} options={MEAL_TYPE_OPTIONS} placeholder="Seleccione..." error={errors.mealType} />
            <Input label="Porciones base" type="number" value={String(values.baseServings)} onChange={handleChange('baseServings')} error={errors.baseServings} />
          </div>
