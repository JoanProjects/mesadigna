...
import { Loader, Card, Badge, Button, Input, Select, Alert } from '@/components/ui';
...
export default function MealDetailPage() {
  ...
  const { values, errors, serverError, setServerError, validate, handleChange, reset } = useForm(mealIngredientSchema, {
    ...
    <>
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        <div>
          ...
          <p className="text-sm text-text-secondary mt-1"><Badge variant="info">{meal.mealType}</Badge> &middot; {meal.baseServings} porciones base</p>
        </div>
        <Link to={`/comidas/${id}/editar`}><Button variant="secondary" size="sm"><FontAwesomeIcon icon={faPen} className="mr-1.5" />Editar</Button></Link>
      </div>
              ...
              <tbody>{meal.ingredients.map(mi => (
                <tr key={mi.id} className="border-b border-gray-50">
                  ...
                  <td className="px-4 py-3 text-sm">
                    <button onClick={() => setConfirmRemove(mi.ingredientId)} className="p-1.5 rounded-lg hover:bg-danger-50 text-text-secondary hover:text-danger-500 transition-colors cursor-pointer border-none bg-transparent" title="Eliminar">
        ...
        <div className="mt-4">
          {showAddForm ? (
            <form onSubmit={addIngredient} className="space-y-3 p-4 bg-gray-50 rounded-lg">
              ...
              <Select label="Ingrediente" value={String(values.ingredientId)} onChange={handleChange('ingredientId')} options={ingredientOptions} placeholder="Seleccione..." error={errors.ingredientId} />
              <div className="grid grid-cols-2 gap-3">
                <Input label="Cantidad por porción" type="number" step="0.01" value={String(values.quantityPerServing)} onChange={handleChange('quantityPerServing')} error={errors.quantityPerServing} />
                <Select label="Unidad" value={values.unitOfMeasure} onChange={handleChange('unitOfMeasure')} options={UNIT_OPTIONS} placeholder="Seleccione..." error={errors.unitOfMeasure} />
              </div>
              <div className="flex gap-2">
                <Button type="submit" size="sm" loading={saving}>Agregar</Button>
                <Button type="button" variant="secondary" size="sm" onClick={() => { setShowAddForm(false); reset(); }}>Cancelar</Button>
              </div>
            </form>
          ) : (
            <Button variant="secondary" size="sm" onClick={() => setShowAddForm(true)}><FontAwesomeIcon icon={faPlus} className="mr-1.5" />Agregar ingrediente</Button>
          )}
        </div>
      </Card>

      <ConfirmDialog open={!!confirmRemove} title="Eliminar ingrediente" message="¿Está seguro de eliminar este ingrediente de la comida?" confirmText="Eliminar" onConfirm={removeIngredient} onCancel={() => setConfirmRemove(null)} />
    </>
  );
}
