        ...
        <>
            ...
            <Card>
                <form onSubmit={onSubmit} className="space-y-4 max-w-2xl">
                    ...
                    <Select label="Unidad de medida" value={values.unitOfMeasure}
                            onChange={handleChange('unitOfMeasure')} options={UNIT_OPTIONS} placeholder="Seleccione..."
                            error={errors.unitOfMeasure}/>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <Input label="Cantidad en stock" type="number" min="0" step="0.01"
                               value={String(values.stockQuantity)} onChange={handleChange('stockQuantity')}
                               error={errors.stockQuantity}/>
                        <Input label="Stock mínimo" type="number" min="0" step="0.01"
                               value={String(values.minimumStock)} onChange={handleChange('minimumStock')}
                               error={errors.minimumStock}/>
                    </div>
                    <div className="flex gap-3 pt-2">
                        <Button type="submit"
                                loading={saving}>{isEdit ? 'Guardar cambios' : 'Crear ingrediente'}</Button>
