...
export default function ProfilePage() {
    ...
    const onSaveProfile = async (e: React.FormEvent) => {
        ...
        profile.setServerError(null);
        const valid = await profile.validate();
        if (!valid) {
        ...
        try {
            ...
            if (err instanceof ApiError) {
                ...
                if (Object.keys(fe).length > 0) profile.setErrors(fe);
                profile.setServerError(getErrorMessage(err));
            }
        } finally {
    ...
    const onChangePassword = async (e: React.FormEvent) => {
        ...
        password.setServerError(null);
        const valid = await password.validate();
        if (!valid) {
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <Card title="Datos personales">
                    <form onSubmit={onSaveProfile} className="space-y-4">
                        ...
                        <Input label="Nombre" value={profile.values.firstName}
                               onChange={profile.handleChange('firstName')} error={profile.errors.firstName}/>
                        <Input label="Apellido" value={profile.values.lastName}
                               onChange={profile.handleChange('lastName')} error={profile.errors.lastName}/>
                        <Input label="Correo electrónico" type="email" value={profile.values.email}
                               onChange={profile.handleChange('email')} error={profile.errors.email}/>
                        <Input label="Teléfono" value={profile.values.phoneNumber ?? ''}
                               onChange={profile.handleChange('phoneNumber')} error={profile.errors.phoneNumber}/>
                ...
                <Card title="Cambiar contraseña">
                    <form onSubmit={onChangePassword} className="space-y-4">
                        ...
                        <Input label="Contraseña actual" type="password" value={password.values.currentPassword}
                               onChange={password.handleChange('currentPassword')}
                               error={password.errors.currentPassword} autoComplete="current-password"/>
                        <Input label="Nueva contraseña" type="password" value={password.values.newPassword}
                               onChange={password.handleChange('newPassword')} error={password.errors.newPassword}
                               autoComplete="new-password"/>
                        <Input label="Confirmar contraseña" type="password" value={password.values.confirmPassword}
                               onChange={password.handleChange('confirmPassword')}
                               error={password.errors.confirmPassword} autoComplete="new-password"/>
